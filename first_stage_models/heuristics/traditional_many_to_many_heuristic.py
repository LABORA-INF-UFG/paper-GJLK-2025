...
def heuristic(n_CNs, n_GNBs, n_users, numerology, timestamp):
    ...
    users_desired_resolution = {}
    users_desired_frame_rate = {}
    for user in Users:
        if users_priority[user.my_ID()] == 1:
            users_desired_resolution[user.my_ID()] = user.my_max_resolution()
            users_desired_frame_rate[user.my_ID()] = 30
        else:
            users_desired_resolution[user.my_ID()] = "1080p"
            users_desired_frame_rate[user.my_ID()] = user.my_max_frame_rate()
    ...
    while len(users_associated_gNBs) < len(Users):
        for user in Users:
            ...
            if user.my_ID() in users_associated_gNBs.keys():
                ...
                if len(users_associated_gNBs[user.my_ID()]) == 3:
                    continue
            else:
            ...
            for gnb in users_preference_list[user.my_ID()]:
                if len(users_associated_gNBs[user.my_ID()]) == 3:
                    continue
                if user_PRB_request[user.my_ID()][gnb]["1080p"][30]/max(1, len(users_associated_gNBs[user.my_ID()])) <= tmp_gNBs_available_PRBs[gnb]:
                        ...
                        tmp_gNBs_available_PRBs[gnb] -= user_PRB_request[user.my_ID()][gnb]["1080p"][30]/max(1, len(users_associated_gNBs[user.my_ID()]))
                    ...
                    if gnb in gNBs_association.keys():
                        for u in gNBs_association[gnb]:
                            if users_priority[user.my_ID()] > users_priority[u]:
                                ...
                                if user_PRB_request[user.my_ID()][gnb]["1080p"][30]/max(1, len(users_associated_gNBs[user.my_ID()])) <= tmp_gNBs_available_PRBs[gnb] + user_PRB_request[u][gnb]["1080p"][30]/max(1, len(users_associated_gNBs[u])):
                                    # update the available PRBs of the gNB
                                    tmp_gNBs_available_PRBs[gnb] += user_PRB_request[u][gnb]["1080p"][30]/max(1, len(users_associated_gNBs[u])) - user_PRB_request[user.my_ID()][gnb]["1080p"][30]/max(1, len(users_associated_gNBs[user.my_ID()]))
            ...
            for gNB in users_associated_gNBs[user.my_ID()]:
                ...
                # gNBs_available_PRBs[gNB] = gNBs_available_PRBs[gNB] - user_PRB_request[user.my_ID()][gnb]["1080p"][30]/max(1, len(users_associated_gNBs[user.my_ID()]))
                gNBs_available_PRBs[gNB] = tmp_gNBs_available_PRBs[gNB]
    ...
    json.dump(solution_json, open("../../solutions/first_stage/traditional_heuristic/{}_CNs_{}_gNBs_{}_users_timestamp_{}.json".format(n_CNs, n_GNBs, n_users, timestamp), "w"), indent=4)
...
if __name__ == "__main__":
    ...
    for timestamp in range(0, 100):
        heuristic(n_CNs, n_GNBs, n_users, numerology, timestamp=timestamp)