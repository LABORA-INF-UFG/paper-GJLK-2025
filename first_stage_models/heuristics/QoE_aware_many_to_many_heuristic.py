...
def heuristic(n_CNs, n_GNBs, n_users, numerology, timestamp):
    ...
    n_connections = 3 # maximum number of connections per user
    ...
    while len(users_associated_gNBs) < len(Users):
        for user in Users:
            ...
            # if the user is associated with any gNB, it checks if it reaches the maximum number of connections. If it do not, it will try to associate with the gNB that has higher preference
            if user.my_ID() in users_associated_gNBs.keys():
                # if the user is associated with the max number of gNBs, it will not try to associate with any other gNB
                if len(users_associated_gNBs[user.my_ID()]) == n_connections:
                    continue
            else:
    ...
    for user in Users:
        output_users.append({
            "ID": user.ID, 
            "serving_gNBs": users_associated_gNBs[user.ID],
            "Total_PRBs": sum(user_PRB_request[user.ID][gnb][users_selected_resolution[user.ID]][users_selected_frame_rate[user.ID]] for gnb in users_associated_gNBs[user.ID]),
            "PRBs": {gnb: int(math.ceil(user_PRB_request[user.ID][gnb][users_selected_resolution[user.ID]][users_selected_frame_rate[user.ID]] / max(1, len(users_associated_gNBs[user.ID])))) for gnb in users_associated_gNBs[user.ID]},
            "total_load": sum(user_PRB_request[user.ID][gnb][users_selected_resolution[user.ID]][users_selected_frame_rate[user.ID]]/max(1, len(users_associated_gNBs[user.ID])) for gnb in users_associated_gNBs[user.ID] for gnb in users_associated_gNBs[user.ID]),
            "load_to_gNB": {gnb: user_PRB_request[user.ID][gnb][users_selected_resolution[user.ID]][users_selected_frame_rate[user.ID]]/max(1, len(users_associated_gNBs[user.ID])) for gnb in users_associated_gNBs[user.ID]},
            "total_throughput": sum(user_PRB_request[user.ID][gnb][users_selected_resolution[user.ID]][users_selected_frame_rate[user.ID]] * GNBs[gnb - 1].my_PRB_BW() * 10 ** 6 * user.my_SE(gnb) for gnb in users_associated_gNBs[user.ID]),
            "throughput_to_gNB": {gnb: user_PRB_request[user.ID][gnb][users_selected_resolution[user.ID]][users_selected_frame_rate[user.ID]] * GNBs[gnb - 1].my_PRB_BW() * 10 ** 6 * user.my_SE(gnb) for gnb in users_associated_gNBs[user.ID]},
            "latency_to_gNB": {gnb: (user_PRB_request[user.ID][gnb][users_selected_resolution[user.ID]][users_selected_frame_rate[user.ID]]/max(1, len(users_associated_gNBs[user.ID]))) / (user_PRB_request[user.ID][gnb][users_selected_resolution[user.ID]][users_selected_frame_rate[user.ID]] * GNBs[gnb - 1].my_PRB_BW() * 10 ** 6 * user.my_SE(gnb)) + user_routing_latency for gnb in users_associated_gNBs[user.ID]},
            "gNB_PRBs": {gnb: int(math.ceil(user_PRB_request[user.ID][gnb][users_selected_resolution[user.ID]][users_selected_frame_rate[user.ID]] / max(1, len(users_associated_gNBs[user.ID])))) for gnb in users_associated_gNBs[user.ID]},
            "resolution": users_selected_resolution[user.ID],
            "frame_rate": users_selected_frame_rate[user.ID],
            "priority": "quality" if users_priority[user.ID] == 1 else "performance",
            "QoE": users_QoE[user.ID]})
    
    solution_json = {"solution": {"Total_QoE": total_QoE, "time": elapsed_time, "users": output_users}}
    
    json.dump(solution_json, open("../../solutions/first_stage/QoE_aware_heuristic/{}_CNs_{}_gNBs_{}_users_timestamp_{}.json".format(n_CNs, n_GNBs, n_users, timestamp), "w"), indent=4)
...
if __name__ == "__main__":
    ...
    # json.dump("../../solutions/first_stage/QoE_aware_heuristic/time.json", run_times, indent=4)