...
def heuristic(n_CNs, n_GNBs, n_users, numerology, timestamp):
    ...
    while len(users_associated_gNBs) < len(Users):
        for user in Users:
            ...
            if user.my_ID() in users_associated_gNBs.keys():
                ...
                if len(users_associated_gNBs[user.my_ID()]) == 2:
                    continue
            else:
    ...
    json.dump(solution_json, open("../../solutions/first_stage/dual_connectivity_model/{}_CNs_{}_gNBs_{}_users_timestamp_{}.json".format(n_CNs, n_GNBs, n_users, timestamp), "w"), indent=4)
