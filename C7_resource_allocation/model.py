...
def run_model(BSs, users, MEC_servers, image_ID):
    model = CpoModel()
    myparams = CpoParameters(LogPeriod=9999999999, SearchType="IterativeDiving", TimeLimit=60*5)
    ...
    user_device_max_resolution = {}

    for user in users:
        if user.device["resolution"] == "2k":
            user_device_max_resolution[user.ID] = 4
        elif user.device["resolution"] == "4k":
            user_device_max_resolution[user.ID] = 8
        elif user.device["resolution"] == "8k":
            user_device_max_resolution[user.ID] = 24
    
    for user in users:
        if user.device["resolution"] in ["2k", "4k", "8k"]: # se resolução do usuário for 2k, então ele pode ser admitido em 4k
        ...
        if user.device["resolution"] in ["4k", "8k"]: # se for 4k, pode ser admitido em 4k e 8k
            for object in objects[user.ID]:
                i_resolution_user_object.append((user.ID, object.ID, 8))
        if user.device["resolution"] in ["8k"]: # se for 8k, pode ser admitido em 4k, 8k, 12k e 24k
            for object in objects[user.ID]:
                i_resolution_user_object.append((user.ID, object.ID, 12))
                i_resolution_user_object.append((user.ID, object.ID, 24))
    
    i_FPS_user = [] # iterador entre USUARIOS e FPS

    for user in users:
        if user.device["fps"] in [30, 60, 90]:
            i_FPS_user.append((user.ID, 30))
        # if user.device["fps"] in [60, 90]:
        #     i_FPS_user.append((user.ID, 60))
        # if user.device["fps"] in [90]:
        #     i_FPS_user.append((user.ID, 90))
    ...
    print(processing_latency, propagation_latency, queueing_latency)
    # função objetivo    
    # model.sum(model.w_user_object[i] * log(i[2]/2) * users_attention[i[0]][i[1]] for i in i_resolution_user_object if i[0] == user.ID)

    model.maximize(model.sum((((1/model.sum(model.u_user[user.ID, fps] * fps for fps in [30, 60, 90] if (user.ID, fps) in i_FPS_user)) - # (Tmax - 
                                ((model.sum(model.sum(model.w_user_object[user.ID, obj.ID, res] * obj.length * pixel_scale[res] * bit_per_pixel_compression for res in resolution_vector if (user.ID, obj.ID, res) in i_resolution_user_object) for obj in objects[user.ID]) / # (Tk) Load /
                                  model.sum(model.x_user[bs.ID, user.ID] * 4 * log(1 + (10**(user.my_SINR(bs.ID)/10)), 2) for bs in BSs)) + # (Tk) Load / TP +
                              propagation_latency + processing_latency + queueing_latency) # (tk): Load/TP + prop + proc + queue + routing
                              )/(1/model.sum(model.u_user[user.ID, fps] * fps for fps in [30, 60, 90] if (user.ID, fps) in i_FPS_user))) *
    ...
    model.add_constraint(model.sum((((1/model.sum(model.u_user[user.ID, fps] * fps for fps in [30, 60, 90] if (user.ID, fps) in i_FPS_user)) - # (Tmax - 
                                ((model.sum(model.sum(model.w_user_object[user.ID, obj.ID, res] * obj.length * pixel_scale[res] * bit_per_pixel_compression for res in resolution_vector if (user.ID, obj.ID, res) in i_resolution_user_object) for obj in objects[user.ID]) / # (Tk) Load /
                                  model.sum(model.x_user[bs.ID, user.ID] * 4 * log(1 + (10**(user.my_SINR(bs.ID)/10)), 2) for bs in BSs)) + # (Tk) Load / TP +
                              propagation_latency + processing_latency + queueing_latency) # (tk): Load/TP + prop + proc + queue + routing
                              )/(1/model.sum(model.u_user[user.ID, fps] * fps for fps in [30, 60, 90] if (user.ID, fps) in i_FPS_user))) *
                              ((model.sum(model.x_user[bs.ID, user.ID] * 4 * log(1 + (10**(user.my_SINR(bs.ID)/10)), 2) for bs in BSs))/
                              (((100e6) * 4 * log(1 + (10**(30/10)), 2)))) *
                               model.sum(model.w_user_object[i] * log(i[2]/2) * users_attention[i[0]][i[1]] for i in i_resolution_user_object if i[0] == user.ID) for user in users) <= model.sum(log(24/2) * users_attention[user.ID][obj.ID] for user in users for obj in objects[user.ID]))

    for user in users:
        model.add_constraint(((model.sum(model.sum(model.w_user_object[user.ID, obj.ID, res] * obj.length * pixel_scale[res] * bit_per_pixel_compression for res in resolution_vector if (user.ID, obj.ID, res) in i_resolution_user_object) for obj in objects[user.ID]) / # (Tk) Load /
                                  model.sum(model.x_user[bs.ID, user.ID] * 4 * log(1 + (10**(user.my_SINR(bs.ID)/10)), 2) for bs in BSs)) + # (Tk) Load / TP +
                              propagation_latency + processing_latency + queueing_latency) >= 0)
    ...
    # for user in users:
    #     model.add_constraint(model.sum(model.x_user[bs.ID, user.ID] for bs in BSs) * 4 * log(1 + (10**(user.my_SINR(bs.ID)/10)), 2) 
    #                          >= model.sum(obj.length * pixel_scale[2] * bit_per_pixel_compression for obj in objects[user.ID])/1000)

    # # Latency constraint of users in ms 
    # for user in users:
    #     model.add_constraint(model.sum(model.sum(model.w_user_object[user.ID, obj.ID, res] * obj.length * pixel_scale[res] * bit_per_pixel_compression for res in resolution_vector if (user.ID, obj.ID, res) in i_resolution_user_object)/1000 for obj in objects[user.ID]) + 
    #                         (model.sum(model.x_user[bs.ID, user.ID] for bs in BSs) * 4 * log(1 + (10**(user.my_SINR(bs.ID)/10)), 2)) *
    #                          model.sum(model.z_user[mec.ID, user.ID] * model.sum(model.x_user[bs.ID, user.ID] * mec.latency["BS_{}".format(bs.ID)] for bs in BSs) for mec in MEC_servers) <= (model.sum(model.x_user[bs.ID, user.ID] for bs in BSs) * 4 * log(1 + (10**(user.my_SINR(bs.ID)/10)), 2)) * (model.sum(model.u_user[user.ID, fps] * 1000/fps for fps in [30, 60, 90] if (user.ID, fps) in i_FPS_user) - (propagation_latency + processing_latency + queueing_latency)))
    ...
    for i in i_resolution_user_object:
        if msol[model.w_user_object[i]] > 0.01:
            ...
            # print("User {} is using resolution {} for object {} with length {} and attention {}, load {}".format(i[0], i[2], i[1], obj_length[i[1]], users_attention[i[0]][i[1]], obj_length[i[1]] * pixel_scale[i[2]] * bit_per_pixel_compression))
    ...
    # print(user_total_load)
    solution_json["Solution"]["Users_Render_QoE"] = {}
    for user in users:
        solution_json["Solution"]["Users_Render_QoE"]["User_{}".format(user.ID)] = user_qoe[user.ID]
    ...
    for i in i_BS_user:
        ...
        if msol[model.y_user[i]] > 0.01:
            ...
            # print("BS {}: Serving {} Hz for User {}, with {:.1f} Mbps".format(i[0], 
            #                                                      msol[model.x_user[i]],
            #                                                      i[1],
    ...
    for user in users:
        connection_coeff_TP = (user_vazao[user.ID]/(((100e6) * 4 * log(1 + (10**(30/10)), 2))))
        connection_coeff_Lat = (1/30 - ((user_total_load["User_{}".format(user.ID)]/user_vazao[user.ID]) + propagation_latency + processing_latency + queueing_latency)) / (1/30)
        
        solution_json["Solution"]["Users_QoE"]["User_{}".format(user.ID)] = connection_coeff_Lat * connection_coeff_TP * user_qoe[user.ID]
        qoe_sum += connection_coeff_Lat * connection_coeff_TP * user_qoe[user.ID]
    ...
    count = 0
    for user in user_vazao:
        count += user_total_load["User_{}".format(user)] / user_vazao[user] + 0.001024 + 0 + 0.1
        # print("User {} latency: ".format(user), user_total_load["User_{}".format(user)] / user_vazao[user] + 0.001024 + 0 + 0.1)
        # print(user_vazao[user])