...
for user in range(1, 17):
    ...
    for i in json_obj["Solution"]["Users_QoE"]:
        C7_latency[user].append(json_obj["Solution"]["Users_QoE"][i])

# Prepare data for plotting
users = list(range(1, 17))

y1 = [raj_jain_fairness(latency[user]) * 100 for user in users]
C7_y1 = [raj_jain_fairness(C7_latency[user]) * 100 for user in users]
...
for user in range(1, 17):
    ...
    for i in json_obj["Solution"]["BS_user_association"]:
        C7_QoE[user].append(i["Latency"])

# Prepare data for plotting
users = list(range(1, 17))
y2 = [raj_jain_fairness(QoE[user]) * 100 for user in users]
C7_y2 = [raj_jain_fairness(C7_QoE[user]) * 100 for user in users]
...
for user in range(1, 17):
    ...
    for i in json_obj["Solution"]["Users_QoE"]:
            if json_obj["Solution"]["Users_QoE"][i] > 0:
                C7_count += 1
    C7_Admission.append((C7_count * 100)/user)
...
ax1.plot(x[4:], C7_y1[4:], marker='o', markersize=8, fillstyle="none", linestyle='--', color='firebrick', markerfacecolor='white')
ax1.plot(x[:5], C7_y1[:5], marker='o', markersize=8, linestyle='--', color='firebrick')
...
ax2.plot(x[4:], C7_y2[4:], marker='o', markersize=8, fillstyle="none", linestyle='--', color='firebrick', markerfacecolor='white')
ax2.plot(x[:5], C7_y2[:5], marker='o', markersize=8, linestyle='--', color='firebrick')
...
ax3.plot(x[4:], C7_y3[4:], marker='o', markersize=8, fillstyle="none", linestyle='--', color='firebrick', markerfacecolor='white')
ax3.plot(x[:5], C7_y3[:5], marker='o', markersize=8, linestyle='--', color='firebrick')
