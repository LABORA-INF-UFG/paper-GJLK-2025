...
def model(n_CNs, n_GNBs, n_users, timestamp):
    ...
    # routing latency considered in the first stage to the closest CN
    ...
    for user in Users:
        for p in Paths:
            if str(p.destination) not in users_load_to_gNB[user.my_ID()].keys():
    ...
    if model.solution is not None:
        ...
        for p in Paths:
            for u in Users:
                user_latency = 0
                if model.s[p.ID, u.ID].solution_value >= 0.999:
        ...
        json.dump(solution_json, open("../solutions/second_stage/single_path_model/{}_CNs_{}_gNBs_{}_users_timestamp_{}.json".format(n_CNs, n_GNBs, n_users, timestamp), 'w'), indent=4)
    else:
        print("No solution found")
        json.dump({"solution": "Not found"}, open("../solutions/second_stage/single_path_model/{}_CNs_{}_gNBs_{}_users_timestamp_{}.json".format(n_CNs, n_GNBs, n_users, timestamp), 'w'), indent=4)
