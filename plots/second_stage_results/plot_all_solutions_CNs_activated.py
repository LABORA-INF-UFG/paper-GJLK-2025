...
for timestamp in x:
    ...
    for user in users:
        if users[user]["used_CN"] not in activated_CNs:
            activated_CNs.append(users[user]["used_CN"])
    y_optimal.append(len(activated_CNs))
    ...
    for user in users:
        if users[user]["used_CN"] not in activated_CNs:
            activated_CNs.append(users[user]["used_CN"])
    y_single_path.append(len(activated_CNs))
    ...
    for user in users:
        if users[user]["used_CN"] not in activated_CNs:
            activated_CNs.append(users[user]["used_CN"])
    y_unconstrained.append(len(activated_CNs))
...
plt.ylabel("Activated CNs (#)", fontsize=fontsize)
plt.xlim(0, 100)
plt.ylim(0, 10)
# plt.legend()
plt.yticks([_ for _ in range(0, 11)], fontsize=fontsize)
...
plt.tight_layout()
plt.savefig("comparing_CNs_active.pdf")
# plt.show()